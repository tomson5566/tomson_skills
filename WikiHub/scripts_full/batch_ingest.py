...
"""
WikiHub Script: batch_ingest

Purpose:
- Promote multiple inbox items into the wiki in one deterministic run.

Usage:
- Prefer `python scripts/wikihub batch-ingest ...`.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Batch ingest inbox items into WikiHub, prioritizing quality-filtered entries."
    )
    ...
    lines.extend([
        ...
        f"Next: run `python scripts/wikihub viewer --root {root}` to refresh the local viewer page.",
        f"Next: run `python scripts/wikihub graph --root {root}` to refresh the knowledge graph page.",
    ])
    print("\n".join(lines))
