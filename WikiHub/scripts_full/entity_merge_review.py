...
"""
WikiHub Script: entity_merge_review

Purpose:
- Generate review artifacts for ambiguous entity alias groups in the current graph.

Usage:
- Prefer `python scripts/wikihub entity-merge-review ...`.
...
def _load_graph(root: Path) -> dict[str, object]:
    ...
    if not graph_path.exists():
        raise SystemExit("Graph data not found. Run `python scripts/wikihub graph --root <wiki-root>` first.")
    try:
...
<head>
  ...
  <title>WikiHub Entity Merge Review</title>
  <style>
    :root {{
      --bg: #0b1020;
      --panel: #121935;
...
<body>
  <main class="shell">
    <h1>WikiHub Entity Merge Review</h1>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a deterministic entity merge review from the current WikiHub graph data.")
    ...
    print("# WikiHub Entity Merge Review")
    print("")
    print(f"- Root: {root}")
