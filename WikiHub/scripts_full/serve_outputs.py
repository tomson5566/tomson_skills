...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Serve WikiHub HTML outputs over loopback HTTP for browser access."
    )
    ...
    if args.host not in loopback_hosts:
        print(
            f"Warning: serving WikiHub outputs on {args.host}:{args.port or DEFAULT_SERVE_PORT} "
    ...
    if not output_dir.is_dir():
        print(
            ...
            f"Run `python scripts/wikihub viewer --root {display_root_arg(root)}` "
