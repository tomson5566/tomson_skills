...
"""
WikiHub Script: entity_merge_apply

Purpose:
- Preview or apply a deterministic entity merge into a canonical entity page.

Usage:
- Prefer `python scripts/wikihub entity-merge-apply ...`.
...
def _build_merge_plan(
    *,
    ...
    if not str(canonical_after_meta.get("summary") or "").strip():
        canonical_after_meta["summary"] = f"{canonical_title} is an entity tracked in WikiHub."
...
<head>
  ...
  <title>WikiHub Entity Merge Plan</title>
  <style>
    :root {{
      --bg: #0b1020;
      --panel: #121935;
...
<body>
  <main class="shell">
    <h1>WikiHub Entity Merge Plan</h1>
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        print("# WikiHub Entity Merge Plan")
        print("")
        print(f"- Root: {root}")
    ...
    print("# WikiHub Entity Merge Apply")
    print("")
    print(f"- Root: {root}")
    print(f"- Identity Key: {args.identity_key}")
    print(f"- Canonical: {canonical_entity_id}")
