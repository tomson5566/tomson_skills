...
def extract_title_from_body(body: str, fallback: str) -> str:
    """Extract the first H1 heading from markdown body, or return fallback."""
...
def resolve_input_path(root: Path, value: str) -> Path:
    """Resolve a potentially relative path against the wiki root."""
    path = Path(value)
    if path.is_absolute():
        return path
...
def source_page_companion(root: Path, page_path: Path) -> "Path | None":
    """Find the normalized companion for a source wiki page, if any."""
...
def collect_related_paths(root: Path, body: str, page_path: Path) -> list[str]:
    """Collect relative paths to related wiki pages referenced in markdown body."""
    related: list[str] = []
...
def format_output_serve_lines(
    ...
    *,
    ...
    lines = [
        f"WikiHub output server: {output_http_base(host, port)}",
...
def output_serve_hint_line(root: Path) -> str | None:
    ...
    return (
        "Browse via HTTP: "
        f"run `python scripts/wikihub serve --root {display_root_arg(root)}` "
...
def collect_inbox_items(root: Path) -> list[dict]:
    ...
    for path in paths:
        ...
        items.append({
            ...
            "ingest_command": f"python scripts/wikihub ingest --root {display_root_arg(root)} --source {repo_path}",
            "kind": str(sidecar.get("kind", "") or ""),
            "adapter": str(sidecar.get("adapter", "") or ""),
            "site_name": str(sidecar.get("siteName", "") or ""),
            "author": str(sidecar.get("author", "") or ""),
...
def batch_ingest_command(root: Path, quality: str = "ready", limit: int | None = None, dry_run: bool = False) -> str:
    command = f"python scripts/wikihub batch-ingest --root {display_root_arg(root)} --quality {quality}"
...
def write_inbox_review(root: Path) -> Path:
    ...
    ]) if items else "<div class='empty'>There are no inbox items yet. Run `python scripts/wikihub clip ...` to collect webpages, text, or files first.</div>"
    html = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>WikiHub Inbox Review</title>
  <style>
    :root {{
      --bg: #0b1020;
      --panel: #121935;
...
def output_access_lines(root: Path) -> list[str]:
    ...
    if output_home is not None:
        ...
        lines.append(
            "Browse via HTTP: "
            f"`python scripts/wikihub serve --root {display_root_arg(root)}` "
            f"-> {output_http_url('index.html')}"
        )
        if viewer_exists and not graph_exists:
            lines.append(f"Next: run `python scripts/wikihub graph --root {root}` to generate the graph page.")
        elif graph_exists and not viewer_exists:
            lines.append(f"Next: run `python scripts/wikihub viewer --root {root}` to generate the viewer page.")
        return lines

    lines.append(f"Next: run `python scripts/wikihub viewer --root {root}` to generate the local viewer page.")
    lines.append(f"Next: run `python scripts/wikihub graph --root {root}` to generate the knowledge graph page.")
    return lines
